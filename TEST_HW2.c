...
serie** getUnifyDatabaseDetails(FILE* fp, FILE* watch, int number_series) {
    ...
    while (!feof(fp)) {
        ...
        //printf("serieName:%s\nID: %d\nnumSeasons: %d\n", S[i]->serieName, S[i]->ID, S[i]->numSeasons);
        S[i]->rank = 0;
...
void getWatchingDetails(serie** seriesData, int num) {
    ...
    if (flagSerieCode == 0) {
        printf("worng serie code, try again!!!\n");
    ...
    do {
        ...
        if (1 <= SeasonNumber && SeasonNumber <= rangeSeason) {
            ...
            //        fprintf(watch, "Series Code:%03d, Season Number:%d\n", flagSerieCode, SeasonNumber);
            fprintf(watch, "%03d,%d\n", flagSerieCode, SeasonNumber);
            //        printf("data written");
            //numseries = LenNumSeries(num);
...
void printWatchingDetails() {
    ...
    //int number_series = LenNumSeries(watch);
...
void printSeriesByRank(serie** list, int num) {
    ...
    for (int i = num-1; i >= 0; i--) {
        printf("rank %d\n%d - %s\n", list[i]->rank, list[i]->ID, list[i]->serieName);
        for (int k = 0; k < (list[i]->numSeasons);++k) {
            printf("Season %d:%d\n", k+1, list[i]->watchingDetails[k]);
        }
        //for(int j =1; j<=list[i]->numSeasons;j++){
            //printf("Season : %d:%d", j, list[i]->watchingDetails[j]);
    }
    for (int i = 0; i < len - 1; i++) {
        for (int j = 0; j < len - 1 - i; j++) {
